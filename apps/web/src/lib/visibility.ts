...
export const EMPTY_GRAPHICS: ViewGraphics = { categories: {}, elements: {} };
...
export function resolveGraphics(
  elements: ReadonlyArray<{ index: number; category: string }>,
  g: ViewGraphics,
  ...
  for (const e of elements) {
    ...
    if (!c && !el) continue;
    const visible = el?.visible ?? c?.visible ?? true;
    if (!visible) {
      hidden.push(e.index);
      continue;
    }
    const color = el?.color !== undefined ? el.color : c?.color ?? null;
    const transparency = el?.transparency ?? c?.transparency ?? 0;
    const halftone = el?.halftone ?? c?.halftone ?? false;
...
export function countOverrides(g: ViewGraphics): number {
  return Object.values(g.categories).filter((o) => !isEmptyOverride(o)).length + Object.values(g.elements).filter((o) => !isEmptyOverride(o)).length;
}
